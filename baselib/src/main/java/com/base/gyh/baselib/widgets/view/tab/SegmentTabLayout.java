package com.base.gyh.baselib.widgets.view.tab;
...
import com.base.gyh.baselib.utils.FragmentChangeManager;
import com.base.gyh.baselib.widgets.view.helper.UnreadMsgUtils;
...
public class SegmentTabLayout extends FrameLayout implements ValueAnimator.AnimatorUpdateListener {
    ...
    private void obtainAttributes(Context context, AttributeSet attrs) {
        ...
        mBarStartColor = ta.getColor(R.styleable.SegmentTabLayout_tl_bar_start_color, R.color.color_white);
        mBarIsWrap = ta.getBoolean(R.styleable.SegmentTabLayout_tl_bar_isWrap, false);
        mBarEndColor = ta.getColor(R.styleable.SegmentTabLayout_tl_bar_end_color, R.color.color_white);
    ...
    public void setTabData(List<String> titles) {
        String[] strings = (String[]) titles.toArray(new String[titles.size()]);
        setTabData(strings);
    }
    ...
    private void addTab(final int position, View tabView) {
        ...
        /** 每一个Tab的布局参数 */
        LinearLayout.LayoutParams lp_tab = null;
        if (mBarIsWrap) {
            // 每个Tab 的长度是自适应的
    ...
    private void updateTabStyles() {
        for (int i = 0; i < mTabCount; i++) {
            ...
            tv_tab_title.setTextSize(TypedValue.COMPLEX_UNIT_PX, i == mCurrentTab ? mTextsize : mTextsize);
//            tv_tab_title.setPadding((int) mTabPadding, 0, (int) mTabPadding, 0);
    ...
    private void updateTabSelection(int position) {
        for (int i = 0; i < mTabCount; ++i) {
            ...
            tab_title.setTextSize(TypedValue.COMPLEX_UNIT_PX, i == mCurrentTab ? mTextsize : mTextsize);
...
//        Log.d("AAA", "mCurrentP--->" + mCurrentP.left + "&" + mCurrentP.right);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        //draw rect
        @SuppressLint("DrawAllocation") int[] colors = new int[]{mBarStartColor, mBarEndColor};
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
            //俩设置方法其实就是对应着带参构造参数的那俩参数
            mRectDrawable.setOrientation(GradientDrawable.Orientation.LEFT_RIGHT);
            mRectDrawable.setColors(colors);
        } else {
            mRectDrawable.setColor(ContextCompat.getColor(getContext(), mBarStartColor));
        }

...
public class MyGlideUtils {
    ...
    //默认加载              Glide.with(mContext).load(path).into(mImageView);
    public static void loadImageView(Context mContext, String path, ImageView mImageView) {
        Glide.with(mContext).load(path).into(mImageView);
    }
    //默认加载              Glide.with(mContext).load(path).into(mImageView);
    public static void loadImageView(Context mContext, Bitmap path, ImageView mImageView) {
        Glide.with(mContext).load(path).into(mImageView);
    }
    //默认加载
    ...
    //设置加载中以及加载失败图片 为同一张
    public   void loadImageViewLoding(Context mContext, String path, ImageView mImageView, int image) {
        Glide.with(mContext).load(path).transition(DrawableTransitionOptions.withCrossFade()).placeholder(image).error(image).into(mImageView);
    }
